...
Singleton {
  ...
  // Aqui existia uma descarga de saida: `Component.onDestruction` montava o
  // snapshot e gravava com `Quickshell.execDetached`. Ela saiu, e o motivo nao
  // foi a revisao de seguranca - foi que ela NUNCA RODOU.
  //
  // Medido de duas formas independentes, com o save apagado logo antes:
  //
  //   qs -p ... + Qt.exit(0)    o save nao foi recriado
  //   Quickshell.reload()       o save nao foi recriado pelo engine que morreu
  //
  // Ou seja: a promessa de "grava ao descarregar" era falsa desde o primeiro
  // dia, e um caminho que nao executa nao da para revisar nem para testar - e
  // era justamente o unico lancamento destacado do plugin, que a revisao do
  // marketplace apontou por herdar o ambiente. Endurecer codigo morto seria
  // manter a superficie e a promessa; remover resolve os dois.
  //
  // O que garante o save agora, e que sempre foi o que garantia de fato:
  // gravacao em toda acao do usuario, na virada de estagio, ao fechar a sala, e
  // no maximo a cada dez minutos. O pior caso de um encerramento abrupto sao os
  // minutos desde a ultima dessas quatro.
}
